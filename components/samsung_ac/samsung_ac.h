...
namespace esphome
{
  namespace samsung_ac
  {
    ...
    public:
      ...
      void set_discharge_temperature(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
                                 { dev->update_sensor_state(dev->discharge_temperature, value); });
      }

      void set_condenser_mid_temperature(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
                                 { dev->update_sensor_state(dev->condenser_mid_temperature, value); });
      }

      void set_sump_temperature(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
