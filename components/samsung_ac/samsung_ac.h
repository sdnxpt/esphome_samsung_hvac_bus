...
namespace esphome
{
  namespace samsung_ac
  {
    ...
    public:
      ...
      void set_outdoor_discharge_temperature(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
                                 { dev->update_sensor_state(dev->outdoor_discharge_temperature, value); });
      }

      void set_outdoor_condenser_mid_temperature(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
                                 { dev->update_sensor_state(dev->outdoor_condenser_mid_temperature, value); });
      }

      void set_outdoor_operation_mode(const std::string address, float value) override
      {
        execute_if_device_exists(address, [value](Samsung_AC_Device *dev)
                                 { dev->update_sensor_state(dev->outdoor_operation_mode, value); });
      }
